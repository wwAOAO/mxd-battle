...
func (h *Hub) SetInput(playerID string, inputX float64) (Player, bool) {
	now := time.Now().UTC()
...
func (h *Hub) castSkillAt(playerID string, skillID string, now time.Time) (SkillResult, Player, bool) {
	...
	pending := PendingSkill{
		...
		SkillID:   skillID,
		Skill:     skill,
		ReadyAt:   now.Add(combat.SkillStartup(skill)),
		CreatedAt: now,
	}
...
func (h *Hub) StepPhysics(now time.Time, dt float64) {
	...
	for _, room := range h.rooms {
		for playerID, player := range room.players {
			...
			if player.OnGround && player.VY == 0 && player.InputX == 0 {
				if recovered {
					recipients := roomPeers(room)
					playerCopy := player
